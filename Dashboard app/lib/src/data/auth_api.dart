...
class AuthApi {
  ...
  Future<void> updateDanger({required dynamic details}) async {
    final Uri url = Uri.parse('https://dangersapi.azurewebsites.net/update');
    final http.Client client = http.Client();
    final String payload = json.encode(details);
    final Map<String, String> headers = <String, String>{
      'Content-Type': 'application/json',
    };

    await client.put(url, body: payload, headers: headers);
  }

  Future<void> deleteDanger({required dynamic details}) async {
    final Uri url = Uri.parse('https://dangersapi.azurewebsites.net/delete');
    final http.Client client = http.Client();
    final String payload = json.encode(details);
    final Map<String, String> headers = <String, String>{
      'Content-Type': 'application/json',
    ...
    await client.delete(url, body: payload, headers: headers);
  }
  ...
  Future<List<Danger>> getDangers() async {
    ...
    final http.Response response = await client.get(
      Uri.parse(
        'https://dangersapi.azurewebsites.net/getdashboarddangers',
      ),
    );
